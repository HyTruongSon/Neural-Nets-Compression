// Software: Training/Testing Softmax (Neural Network without hidden layer) on MNIST dataset
...
public class Train_Softmax {
	...
	private static void inputParameters() {
		...
		String ModelFileName = "saves/model-Softmax-Epochs-" + Integer.toString(Epochs) + "-LearningRate-" + Double.toString(LearningRate);
		ModelFileNames = new ArrayList<>();
		ModelFileNames.add(ModelFileName + "-Layer-0.dat");

		System.out.println();
		System.out.println("Model file name: " + ModelFileNames.get(0));

		ReportFileName = "saves/report-Softmax-Epochs-" + Integer.toString(Epochs) + "-LearningRate-" + Double.toString(LearningRate) + ".dat";
